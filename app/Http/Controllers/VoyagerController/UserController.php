...
class UserController extends VoyagerBaseController
{
  ...
  public function destroy(Request $request, $id)
  {
    ...
    // Custom 21/6/2024
    $lineage = Lineage::where('user_id', $id)->first();

    if ($lineage) {
      $branches = Branch::where('lineage_id', $lineage->id)->get();

      foreach ($branches as $branch) {
        Page::where('branch_id', $branch->id)->delete();
      }

      Branch::where('lineage_id', $lineage->id)->delete();

      $lineage->delete();

      Temple::where('user_id', $id)->delete();
      ...
      Libary::where('user_id', $id)->delete();

      Announcement::where('user_id', $id)->delete();

      $family_landing_page = FamilyLandingPage::where('user_id', $id)->first();

      $gallery = json_decode($family_landing_page->gallery, true);

      if (!empty($gallery)) {
        foreach ($gallery as $file) {
          Storage::disk('public')->delete($file); // Xóa từng tệp trong gallery
        }

        // Xóa toàn bộ nội dung của gallery
        $gallery = []; // Đặt lại thành mảng rỗng
      ...
      FamilyLandingPage::where('user_id', $id)->delete();
    }
    // End Custom
