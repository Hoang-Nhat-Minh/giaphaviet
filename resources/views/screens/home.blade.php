...
@section('main')
  ...
  @if (empty(Auth::user()->loai_dich_vu))
    <div style="position: fixed; top: 20px; left: 50%; transform: translateX(-50%); z-index: 9999; background: #fff3cd; color: #856404; padding: 10px 24px; border-radius: 30px; border: 1px solid #ffeeba; box-shadow: 0 4px 15px rgba(0,0,0,0.1); font-weight: 600; display: flex; align-items: center; gap: 8px;">
  ...
  <script>
    ...
    family.on('field', function(sender, args) {
      ...
      // console.log(head_of_the_clan)
      // console.log(branch_leader)
    });
      ...
      fetch('{{ route('branch.update', $branch->id) }}', {
        ...
        .then(async response => {
          if (!response.ok) { // If the response fails
            ...
            alert(errorData.message || "Bạn đã đạt tối đa thành viên, hãy nâng cấp tài khoản.");
            window.location.reload();
          }
        });
    });
