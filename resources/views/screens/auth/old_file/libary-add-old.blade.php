        ...
        <div id="video-container">
          <div class="video-item">
            @if ($errors->has('videos'))
              <textarea class="form-control error-placeholder" name="videos" placeholder="{{ $errors->first('videos') }}"></textarea>
            @else
              <textarea class="form-control" name="videos" placeholder="Nhập link video" style="min-height: 300px">{{ old('videos') ?? '' }}</textarea>
            @endif
          </div>
        </div>
      </div>
